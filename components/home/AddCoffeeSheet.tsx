      ...
      <SheetContent>
        ...
        <form onSubmit={handleSubmit} className="px-6 pb-8 space-y-6">
          ...
          <div className="space-y-2">
            <Label>Dónde</Label>
            <div className="flex gap-2">
              {(["CASA", "CAFETERIA"] as ContextType[]).map((c) => (
                <button
                  key={c}
                  type="button"
                  onClick={() => setContextType(c)}
                  className={cn(
                    "flex-1 rounded-xl border py-2.5 text-sm font-medium transition-all",
                    contextType === c
                      ? "border-primary bg-primary text-primary-foreground"
                ...
                >
                  {c === "CASA" ? "🏠 En casa" : "☕ Cafetería"}
                </button>
              ))}
            </div>
            <Input
              placeholder={contextType === "CAFETERIA" ? "Nombre del lugar" : "Marca, cápsula o café"}
              value={contextName}
